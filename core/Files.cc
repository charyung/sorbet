...
PackagedLevel filePackagedSigil(string_view source) {
    size_t start = 0;
    while (true) {
        start = source.find("packaged:", start);
        if (start == string_view::npos) {
            return PackagedLevel::Unspecified;
        }
...
File::File(string &&path_, string &&source_, Type sourceType, uint32_t epoch)
    : epoch(epoch), sourceType(sourceType), flags(path_), path_(move(path_)), source_(move(source_)),
      ...
      compiledLevel(fileCompiledSigil(this->source_)), packagedLevel{filePackagedSigil(this->source_)} {}
...
bool File::isPackaged() const {
    switch (this->packagedLevel) {
        ...
        case PackagedLevel::Unspecified:
            return !this->isRBI();
    }
}
