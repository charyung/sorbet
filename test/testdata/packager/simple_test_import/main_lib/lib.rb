...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ error: Used `test_import` constant `Test::Project::Util::UtilHelper` in non-test file

  Test::Project::Util::Unexported
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ error: Package `Project::Util` does not export `Test::Project::Util::Unexported`
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ error: Used `test_import` constant `Test::Project::Util::Unexported` in non-test file
end
